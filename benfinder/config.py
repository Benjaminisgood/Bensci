...
FETCHER_DEFAULT_USER_AGENT = "benfinder-fetcher/1.0"  # 统一的 UA 字符串，便于识别请求来源
...
# ---------- 模型供应商配置 ----------
# 在 benfinder/extracter_tools/providers.py 注册的 Provider key；可通过 CLI 参数覆盖。
...
# ---------- 可选：覆盖配置（JSON 文件） ----------
# 允许通过 config.override.json 或环境变量 BENFINDER_CONFIG_PATH 提供覆盖值，
# 以便在不修改源码的情况下定制项目配置。
import json
import os
