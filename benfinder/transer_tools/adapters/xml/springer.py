"""Parser for Springer/Nature XML articles."""
...
class SpringerParser(BaseParser):
    ...
    def supports(cls, path, raw_text: str) -> bool:  # type: ignore[override]
        ...
        if "springer" in lowered or "nature.com" in lowered:
            return True
    ...
    def parsing(cls, file_bs) -> List[Paragraph]:  # type: ignore[override]
        ...
        for element in file_bs(cls.all_tags()):
            ...
            if element.name in cls.table_tags:
                ...
                para_type = "text"
                for tag in element(["fig", "table-wrap"]):
                    tag.extract()
                clean_txt = clean_text(element.text)
    ...
    def _is_para(cls, element) -> bool:
        if element.text and element.text.startswith("!ENTITY"):
            return False
        try:
    ...
    def get_metadata(cls, file_bs) -> Metadata:  # type: ignore[override]
        ...
        publisher_tag = file_bs.find("publisher-name")
        journal = clean_text(publisher_tag.text) if publisher_tag and publisher_tag.text else None
        ...
        authors = []
        for contrib in file_bs.find_all("contrib"):
...
register_parser("springer", SpringerParser, priority=30)
